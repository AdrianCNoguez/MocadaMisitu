...
@section('content')
    ...
    <div class="container">
        @section('breadcrumbs')
            <li class="breadcrumb-item text-white mr-3 pl-3" aria-current="page">Resultados de búsqueda</li>
        @endsection
                ...
                <div class="col-10 col-md-3 mb-3 no-gutters">
                  <div class="card border-0 rounded-lg">
                      <div class="img-container p-0">
                          <a href="">
                              <img src="{{ asset('img/'.$item->imagen ) }}" alt="img"s class="obj-img">
                          </a>
                      </div>
  
                      <div class="card-body pl-2 pr-2 pb-0 text-center">
                          <p class="card-title font-weight-bold">{{ $item->nombre }}</p>
                          <a href="{{url($item->link.Str::slug($item->nombre.'-xs'.$item->idProducto))}}" class="btn green mb-3 text-white"><i class="fas fa-eye"></i>Ver detalles</a>
                      </div>
                  </div>
              </div>
            @endforeach
